  ...
  const fetchEmployerData = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:9090/employer/profile/${username}`,
        {
          headers: {
      ...
      if (response.data.empId) {
        try {
          const jobsResponse = await axios.get(
            `http://localhost:9090/employer/getJob/${response.data.empId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        try {
          const appResponse = await axios.get(
            `http://localhost:9090/employer/applications/${response.data.empId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleUpdateApplication = async (applicationId, newStatus) => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:9090/employer/updateApplication/${applicationId}`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <button
            ...
            onClick={() =>
              navigate("/i-dashboard", {
                state: { empId: employer?.empId }, // Passing seekerId to Interview Dashboard
              })
            }
          >
            Interview Dashboard
                  ...
                  <div className="edit-job-form">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditedJob({
                          ...editedJob,
                          noticePeriodInDays: e.target.value,
                        })
                      }
                    ...
                    <input
                      type="text"
                      value={
                        editedJob.requiredSkills
                          ? editedJob.requiredSkills.join(", ")
                          : ""
                      }
                      onChange={(e) =>
                        setEditedJob({
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <div className="job-details">
              ...
              <p>
                <strong>Experience Required:</strong>{" "}
                {selectedJob.experienceInYears} years
              </p>
              <p>
                <strong>Salary:</strong> {selectedJob.jobSalary}
              </p>
              <p>
                <strong>Notice Period:</strong> {selectedJob.noticePeriodInDays}{" "}
                days
              </p>
              <p>
              ...
              <p>
                <strong>Company Email:</strong> {selectedJob.jobCompanyEmail}
              </p>
              <p>
          ...
          <div className="modal-content">
            ...
            <div className="application-details">
              <div className="section">
                ...
                <p>
                  <strong>Salary:</strong> {selectedApplication.jobSalary}
                </p>
                <p>
                  <strong>Experience Required:</strong>{" "}
                  {selectedApplication.experienceInYears} years
                </p>
                <p>
                  <strong>Notice Period:</strong>{" "}
                  {selectedApplication.noticePeriodInDays} days
                </p>
                <p>
                  <strong>Required Skills:</strong>{" "}
                  {selectedApplication.requiredSkills?.join(", ")}
                </p>
                <p>
                ...
                <p>
                  ...
                  {selectedApplication.jobCompanyEmail}
                </p>
                <p>
              ...
              <div className="section">
                ...
                <p>
                  <strong>Name:</strong> {selectedApplication.seekerName}
                </p>
                <p>
                  <strong>Email:</strong> {selectedApplication.seekerEmail}
                </p>
                <p>
                  <strong>Contact:</strong> {selectedApplication.seekerContact}
                </p>
                <p>
                  <strong>Address:</strong> {selectedApplication.seekerAddress}
                </p>
                <p>
                  <strong>Skills:</strong>{" "}
                  {selectedApplication.seekerSkills?.join(", ")}
                </p>
              </div>
