...
const EmployerDashboard = () => {
    ...
    const fetchEmployerData = async () => {
        ...
        try {
            ...
            if (response.data.empId) {
                ...
                // Fetch applications
                try {
                    const appResponse = await axios.get(`http://localhost:9090/employer/getApplications/${response.data.empId}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
    ...
    const fetchApplications = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:9090/employer/applications/${empId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
        ...
        <div className="dashboard-container">
            ...
            <div className="profile-section">
                ...
                <div className="profile-details">
                    <p><strong>Email:</strong> {employer.empEmail}</p>
                    <p><strong>Company:</strong> {employer.empOrg}</p>
                    <p><strong>Contact:</strong> {employer.empContact}</p>
                    <p><strong>Address:</strong> {employer.empAddress}</p>
                </div>
            </div>
                                ...
                                {editingJob?.jobId === job.jobId ? (
                                    ...
                                    <>
                                        ...
                                        <div className="job-actions">
                                            <button onClick={() => handleEditJob(job)} className="edit-btn">Edit Job</button>
                                            <button onClick={() => handleViewDetails(job)} className="view-details-btn">View Details</button>
                                        </div>
                                    </>
                                )}
                            </div>
