...
const JobSeekerDashboard = () => {
  ...
  const fetchSeekerData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:9090/jobseeker/profile/${username}`,
        {
  ...
  const fetchJobs = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:9090/jobseeker/viewAllJobs"
      );
  ...
  const fetchFavoriteJobs = async () => {
    try {
      const response = await axios.get(
        `http://localhost:9090/jobseeker/viewFavouriteJobs/${seeker.jsId}`
      );
  ...
  const addFavoriteJob = async (jobId) => {
    try {
      await axios.post(
        `http://localhost:9090/jobseeker/addFavouriteJob/${jobId}/${seeker.jsId}`
      );
  ...
  const removeFavoriteJob = async (jobId) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:9090/jobseeker/removeFavouriteJob/${jobId}/${seeker.jsId}`
      );
  ...
  const handleSearch = async () => {
    ...
    switch (searchType) {
      ...
      case "byLocation":
        ...
        break;
      case "byExperience":
        endpoint = `http://localhost:9090/jobseeker/searchJobsByExperienceInYears/${parseInt(
          searchText,
          10
        )}`;
        break;
  ...
  const handleApply = async (jobId) => {
    ...
    try {
      await axios.post(
        `http://localhost:9090/jobseeker/applyForJob/${jobId}/${seeker.jsId}`
      );
          ...
          <button
            ...
            onClick={() =>
              navigate("/interview-dashboard", {
                state: { seekerId: seeker?.jsId }, // Passing seekerId to Interview Dashboard
              })
            }
          >
            Interview Dashboard
      ...
      <div className="jobs-section">
        ...
        <div className="search-container">
          ...
          <button
            ...
            onClick={() => {
              const favoriteJobsList = jobs.filter((job) =>
